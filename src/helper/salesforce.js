...
exports.sales_user_details = async (user) => {
    try {
        ...
        let userObject = {
            ...
            MobilePhone: mobile || "-",
            MobileVerified__c: user.hasOwnProperty("mobileVerified") ? user["mobileVerified"] : ( user.__data.mobileVerified ? user.__data.mobileVerified : false ),
            TwoFAVerified__c: user.hasOwnProperty("_2faEnabled") ? user["_2faEnabled"] : (user.__data._2faEnabled ? user.__data._2faEnabled : false),
            ...
            Department: user.__data ? user.__data.department : ( user.department || "-" ),
...
exports.sales_update_user = (user, data) => {
    try {
        ...
        Object.keys(data).forEach(key => {
            // For timestamp values and simple values
            ...
            // For mobile value
            if (key == "mobile") {
                let mobile = (key == "mobile") ? data[key].e164Number.split(data[key].dialCode).join("") : ( data.__data[key] ? data.__data[key].e164Number.split(data.__data[key].dialCode).join("") : null);
                updateObj[salesForceModels[key]] = mobile;
            }
        });
